...
public abstract class ScheduledService extends BaseComposableService<ScheduledService.DelegateService> {
  private UnaryOperator<ScheduledExecutorService> executorServiceDecorator = UnaryOperator.identity();
  ...
  @VisibleForTesting
  public void decorateExecutor(UnaryOperator<ScheduledExecutorService> decorator) {
    checkState(state() == State.NEW, "Tried to apply decorator after start", this);
    this.executorServiceDecorator = decorator;
  }
  ...
  static class DelegateService extends AbstractScheduledService {
    ...
    @Override
    protected ScheduledExecutorService executor() {
      return wrapper.executorServiceDecorator.apply(wrapper.executor());
    }
